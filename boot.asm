...
start:
	...
	mov ah, 0x41	; check extensions for floppy read
	mov dl, 0x00	; drive 0
	mov bx, 0x55aa	; magic 
	int 0x13
	test cl, 1
	
	jz .bypass_supported_operations
	
	mov bp, extended_support
	mov cx, extended_sup_len
	mov bx, 0x000f 				; 01 = first page, 0f = white
	mov dx, 0x0100
	mov ax, 0x1300 				; output a string
	int 0x10
	
	.bypass_supported_operations
	...
	mov ax, 0x0212				; read from floppy ah = 2 - sixteen sectors al = 0x10
	mov dx, 0x0100 				; dx high word is the head (side of the disk, 1 indexed, lies, 0 indexed), low word is the drive  (0 indexed = fda, 1 = fdb)
	mov cx, 0x0001				; cx high word is the track, low word is the sector 
								; this may be because sector 1 has the 0-stage bootloader code, sector 2 is empty (7e00-8000), sector 3 has offset 0x8000+ so 3 to 18 are 16 sectors of data.  
	mov bx, kernel_segment		; clear the segment for the initial assembly reads
	mov es, bx
	mov bx, 0x2000				; bx contains the location to write in the segment es:bx
	int 0x13	 				; read from floppy disk drive
...
section .data
	...
	extended_support	db "Ex. Rd. Supported", 0
	extended_sup_len 	equ $ - extended_support