...
FileSystemInformation * FATPartition::PopulateFileSystemInfo(char * volume_name, byte * boot_sector, dword cluster_count, dword cluster_size, dword num_fat_tables, dword reserved_size, dword volume_id)
{
	return nullptr;
}
...
byte FATPartition::FormatDrive(byte fat_type, char * volume_name, byte * boot_sector_code, dword cluster_count, word cluster_size, byte num_fat_tables, word reserved_size, dword volume_id)
{
	...
	dword fat_table_sectors = (sizeof(dword) * cluster_count) / sector_size;
	...
	{
		dword fat_location = system_sectors + i * num_fat_tables * fat_table_sectors;
...
*/
byte FATPartition::CreateFileAllocationTable(dword fat_location,dword fat_table_sectors)
{
	byte * zero_memory = new byte[sector_size];
	memory_set(zero_memory, sector_size, 0);
